...
type ClientWorkloadInterface interface {
	CreateWorkload(projectID, image, inputDatasetID, pullSecret string, env map[string]string, instances int, useCuteur bool) (output *v1payload.CreateWorkloadOutput, err error)
...
func (c *Client) CreateWorkload(projectID, image, inputDatasetID, pullSecret string, env map[string]string, nrOfWorkers int, useCuteur bool) (output *v1payload.CreateWorkloadOutput, err error) {
